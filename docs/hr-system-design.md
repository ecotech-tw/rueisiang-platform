...
| `POST /api/hr/employments`、`PATCH /api/hr/employments/:id`、`PATCH /api/hr/employments/:id/end`、`POST /api/hr/employments/:id/revoke`、`POST /api/hr/employment-actions/:id/undo` | `hr:employee:write` | 新增／修正日期／結束／復原任職；錯誤任職採保留 `employmentId` 與下游歷史的 soft revoke，不 cascade 刪除或改寫，原任職撤銷後禁止新增關聯，重新加入建立新的任職 |
