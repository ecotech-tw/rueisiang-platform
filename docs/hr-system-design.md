...
| `hr_leave_request_segments` | `leave_request_id, sequence, starts_at, ends_at, requested_seconds` | 未來若需要分段請假再新增；目前單筆請假保存一段台北日期時間與系統計算的 0.5 小時倍數 |
| `hr_leave_requests` | `employment_id → hr_employments.id`、`leave_type_id? → hr_leave_types.id`、`leave_type` snapshot、`status, starts_at, ends_at, starts_on, ends_on, duration_minutes, pay_rate_ppm, reason, reviewed_by?, reviewed_at?, review_comment?, created_by` | `starts_at／ends_at` 以 canonical UTC wall-clock 保存，`starts_on／ends_on` 保留日期覆蓋範圍供出勤與薪資查詢；有已發布班表時依班表逐日工作區段與休息快照計算，其他員工以每個日期的 09:00–18:00 區段扣除 12:00–13:00 午休；班表只保存休息分鐘時，休息優先錨定該日 12:00，不涵蓋時才置中，`pay_rate_ppm` 僅由假別預設值凍結；特休申請需完整落在單一期別；employment + starts_at 索引 |
