...
Portal 沿用 M3 元件，提供手機打卡與班表、週／月員工×日期快速排班、多格套班、差異審核、出勤異常列表與逐項可展開薪資單。AI／Excel 排班輸入先存草稿、來源附件與不確定欄位，經人類發布才生效。HRIS 的管理功能進入 Sidebar 的 HRIS 大分類；員工本人從左下角帳號選單的 HR Information 進入個人入口，模組內再以 Header Nav 分開本人資料與管理功能，避免單頁混合全公司與個人資訊。
