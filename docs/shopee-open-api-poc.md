...
1. 到 Shopee Open Platform 申請開發者帳號，取得 `partner_id` 與 `partner_key`。
   **用公司帳號辦，不要個人帳號**——`partner_id` 掛在個人名下時，那個人離職或帳號出狀況，
   整條串接就斷了。
2. 由店主在瀏覽器完成一次賣場授權，換到 `access_token`（4 小時）與 `refresh_token`（30 天）。
   授權是店主自己登入自己按同意，我們不經手他的帳號密碼，也不會遇到他的簡訊 OTP——
   授權頁存在的意義就是讓我們不必保管別人的憑證。

沒有這兩項，下面的階段二完全跑不動。階段一刻意設計成不需要憑證也能做完。

平台只會有一個蝦皮賣場，所以 scope 維持寫死的 `shopee:store:default`，不做多 `shop_id`。
`partner_id` 本身可以同時持有多家店的 token（第三方 ERP 就是這樣做），但那是另一個題目，
真的要接別人的店時再回來改，現在不預留。
...
**只要 30 天內換過一次就不會過期。** 每次 refresh 蝦皮會同時給新的 `access_token`（4 小時）
與新的 `refresh_token`（30 天重新計時），所以續期是一條不會斷的鏈，不是倒數計時。

### 只能有一個地方 refresh

換完之後舊的 `refresh_token` 就作廢。Worker 換一次、runner 又換一次的話，兩邊會互相把對方
手上的 token 打掉。所以：

- **Worker 是唯一的保管人**：token 存 D1，cron 負責續期。
- **Runner 不保管也不 refresh**：跑報表時跟 Worker 要一個當下有效的 `access_token`。

token **不能放 Worker secret 或 Actions secret**——那些程式改不了，換完寫不回去。

### 續期時機

用既有的 15 分鐘 cron（`apps/api/wrangler.toml` 的 `crons`），`access_token` 剩不到 30 分鐘
就換。一天自動換六次，離 30 天的邊界遠得很。

**不要把續期綁在月報表上。** 只有跑報表時才換的話，中間失敗一次沒人發現，下個月要用時
可能已經過了 30 天。

### 失敗處理

先把新 token 寫進 D1，寫入成功才算換完。舊 `refresh_token` 換完之後有沒有寬限期，官方文件
沒寫，所以要假設沒有：**若蝦皮已經換了但我們寫入失敗，兩邊都會拿不到**，這種情況要留下
紀錄並告警，不可以默默重試。
...
| 單次 refresh 失敗 | 記 log |
| 連續失敗 | 通知（小香或 email） |
| `refresh_token` 剩不到 7 天還沒成功換過 | 明確告警，要有人處理 |

真的過期只有一種解法：**店主再點一次授權連結**。程式繞不過去，所以告警要早。
...
5. **台灣的 app 有沒有分「自用」與「服務商」型、要不要審核。** 官方文件是 JS 網站抓不到
   內文，PDF 只講申請流程。我們只接自己的店，照理不受影響，但申請時要確認清楚。
