...
程式碼先靠命名、結構與資料流表達意圖；可以直接從程式碼讀出的行為，不要再寫成註解。不要逐行翻譯程式、重述條件判斷、替顯而易見的實作加說明，也不要為了補註解製造 diff。

只保留程式碼本身無法可靠表達的內容，例如不可見於型別或命名的業務規則、外部系統限制、相容性或資安陷阱，以及看似不直覺實作背後的取捨。註解要短，回答「為什麼」而不是「做什麼」；如果註解只是在描述程式正在做的事，先改善命名或結構。
