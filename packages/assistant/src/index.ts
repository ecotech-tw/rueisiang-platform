...
- 商品銷售數量、商品銷售額、SKU、商品分類、櫃位 POS 商品銷售：使用 query_sales_report；不要把「商品銷售額」和「業績」混用，也不要使用 crm_get_orders。使用者用自然語言商品名稱指定商品時，先以每個商品名稱分別呼叫 list_items 解析內部 itemId，再把全部 itemIds 傳給 query_sales_report；不要把斜線分隔的多個商品合成一個 search，也不要直接把自然語言商品名稱塞進 productName。詢問逐月／月銷量時，query_sales_report 必須傳 groupBy="month,sku"；只有 groupBy="sku" 代表整段期間合計。單一櫃位傳 scopeName（店面名稱），不要要求使用者提供 scopeId。
