...
function EditDialog({ category, onClose }: { category: ItemCategory; onClose: () => void }) {
  const [name, setName] = useState(category.name);
  const [color, setColor] = useState(category.color);
  const update = useCategoryMutation((input: { id: string; name: string; color: string }) =>
    write(`/api/items/categories/${input.id}`, "PATCH", { name: input.name, color: input.color }),
  );
    ...
    <Dialog
      ...
      formProps={{
        onSubmit: (event) => {
          ...
          if (trimmed) update.mutate({ id: category.id, name: trimmed, color }, { onSuccess: onClose });
        },
      }}
...
export function ItemCategories() {
  ...
  const create = useCategoryMutation((input: { name: string; color: string }) => write("/api/items/categories", "POST", input));
  ...
  return (
    <div className="page fills">
      ...
      <Panel className="grows">
        ...
        {canWrite ? (
          <form className="admin-form toolbar category-form" onSubmit={(event) => {
            ...
            if (!name) return;
            create.mutate({ name, color: nextColor(categories) }, { onSuccess: () => { toast.show(`已新增「${name}」`); setNewName(""); } });
          }}>
          ...
          <table className="data-table category-table">
            ...
            <tbody>
              {categories.map((category) => (
                <tr key={category.id}>
                  <td data-label="分類"><span className={`status status-tone-${category.color}`}>{category.name}</span></td>
      ...
      {editing ? <EditDialog category={editing} onClose={() => setEditing(null)} /> : null}
