...
function SortableCategoryRow({ category, categories, canWrite, over, parentHighlight, onEdit, onDelete, onPromote }: { category: ItemCategory; categories: ItemCategory[]; canWrite: boolean; over: boolean; parentHighlight: boolean; onEdit: () => void; onDelete: () => void; onPromote: () => void }) {
          ...
          <table className="data-table category-table">
            ...
            <DndContext collisionDetection={closestCenter} onDragOver={handleDragOver} onDragCancel={() => { setDraggingId(null); setOverId(null); setDragDeltaX(0); }} onDragEnd={handleDragEnd}>
              <SortableContext items={orderedCategories.map((category) => category.id)} strategy={verticalListSortingStrategy}>
                <tbody>
                  {orderedCategories.map((category) => <SortableCategoryRow key={category.id} category={category} categories={categories} canWrite={canWrite} over={overId === category.id && draggingId !== category.id} parentHighlight={parentHighlightId === category.id && draggingId !== category.id} onEdit={() => setEditing(category)} onDelete={() => setDeleting(category)} onPromote={() => promoteCategory(category)} />)}
                </tbody>
              </SortableContext>
            </DndContext>
          </table>
