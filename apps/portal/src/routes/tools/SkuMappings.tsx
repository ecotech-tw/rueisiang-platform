...
export function SkuMappings() {
  ...
  const adopt = useAdoptCyberbizComponents();
  ...
  const adoptable = data?.adoptableCustomComponents ?? 0;
  ...
  return (
    <div className="page fills">
      <PageHeader
        title="SKU 對應"
        description={<>把通路商品對應到一個以上的組合用料。用料可以是 CYBERBIZ 商品，也可以是不入庫的自訂 SKU——不同通路指到同一個用料，報表就會統計成同一個商品。</>}
        actions={canWrite ? (
          <Button
            icon="plus"
      ...
      {/*
        * 自訂只是官網還沒同步進來時的權宜。同一個 SKU 官網有了之後還留在自訂，名稱就會
        * 停在當初打的那份複本。轉換不改 SKU，報表身分完全不變，所以可以放心整批做。
        */}
      {canWrite && adoptable > 0 ? (
        <Alert tone="info">
          有 {adoptable} 筆自訂用料在 CYBERBIZ 目錄裡找得到。轉成官網商品之後，名稱會跟著官網走，SKU 不變。
          <Button
            variant="secondary"
            disabled={adopt.isPending}
            loading={adopt.isPending}
            loadingLabel="轉換中…"
            onClick={() => adopt.mutate(undefined as never, {
              onSuccess: (result) => toast.show(
                `已轉換 ${result.converted} 筆${result.skippedConflicts ? `，略過 ${result.skippedConflicts} 筆重複` : ""}`,
              ),
            })}
          >
            轉成 CYBERBIZ 商品
      ...
      {adopt.error ? <Alert tone="danger">{adopt.error.message}</Alert> : null}

      {/*
        * 忽略清單收合起來。
        *
        * 它是設定型的東西，一年動不到幾次，但展開時會把列表擠掉半個畫面——
        * 這一頁的主體是那 79 筆對應，版面要留給它。
        */}
      {canWrite ? (
