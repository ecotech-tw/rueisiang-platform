...
function EmployeeCompensationRow({ employee, canWrite }: { employee: Employee; canWrite: boolean }) {
  ...
  if (profile.isLoading) return <tr><td>{employee.displayName}</td><td colSpan={6}>載入敘薪資料…</td></tr>;
  return <>
    <tr>
      ...
      <td>{canWrite && employment ? <Button variant="secondary" onClick={() => setEditing(true)}>新增敘薪</Button> : null}</td>
    </tr>
...
export function HrCompensationManagement({ settingsOnly = false }: { settingsOnly?: boolean } = {}) {
  ...
  return <div className="page">
    ...
    {!settingsOnly ? <Alert tone="info">先在這裡完成員工敘薪，再到「勞健保管理」建立投保版本與「獎金管理」套用業績 policy；最後於「薪資結算」直接計算指定月份薪資。</Alert> : null}
