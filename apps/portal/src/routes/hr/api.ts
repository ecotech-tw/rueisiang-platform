...
export interface HrLeaveRequest { request: { id: string; employmentId: string; leaveTypeId?: string | null; leaveType: string; status: "draft" | "pending" | "approved" | "rejected" | "cancelled"; startsOn: string; endsOn: string; durationMinutes: number; payRatePpm: number; reason: string; reviewedBy: string | null; reviewedAt: string | null; reviewComment: string | null; createdBy: string; createdAt: string }; employeeUserId: string; employeeName: string | null; employeeNumber: string | null; leaveTypeKind?: "annual" | "other" | null }
...
export interface PayrollRun { runId: string; periodKey: string; payDate: string | null; status: "ready" | "closed"; engineVersion: string; employees: PayrollEmployee[]; workers: PayrollWorker[]; warnings: string[] }
...
export interface PayrollRunSummary { run: { id: string; versionNumber: number; status: string; payDate: string | null; engineVersion: string; expectedCount: number; completedCount: number; createdAt: string }; periodKey: string; periodStatus: string }
