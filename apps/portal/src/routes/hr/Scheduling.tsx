...
function ScheduleEntryDialog({ data, day, defaultScopeId, onAdd, onClose }: { data: HrScheduleResponse; day: string; defaultScopeId: string; onAdd: (entry: ScheduleEntry) => void; onClose: () => void }) {
  ...
  const save = useHrWrite();
  const selectedEmployee = data.employees.find((employee) => employee.employmentId === personId);
  const selectedWorker = data.workers.find((worker) => worker.id === personId);
  return <Dialog title={`${day} 新增排班`} onClose={onClose} closeDisabled={save.isPending} formProps={{ onSubmit: (event) => {
    event.preventDefault();
    if (!selectedShift || !personId) return;
    onAdd({ id: `local-${crypto.randomUUID()}`, scheduleVersionId: data.version?.id ?? "", personKind, employmentId: personKind === "employee" ? personId : null, workerId: personKind === "worker" ? personId : null, scopeId, shiftVersionId, workDate: day, startsAt: "", endsAt: "", employeeNumber: personKind === "employee" ? selectedEmployee?.employeeNumber ?? null : null, personName: personKind === "employee" ? selectedEmployee?.name ?? "" : selectedWorker?.name ?? "", scopeName: data.scopes.find((scope) => scope.id === scopeId)?.name ?? "", shiftName: selectedShift.name });
    onClose();
  ...
  return <div className="page fills hr-schedule-page">
    ...
    <Panel className="grows">
      ...
      <div className="hr-calendar">
        ...
        {dates.map((day, index) => <div className="hr-calendar-cell" key={day}>
          <div className="hr-calendar-date"><strong>{index + 1}</strong>{canWrite && !version?.locked ? <button type="button" onClick={() => setAddingDay(day)}>＋</button> : null}</div>
          <div className="hr-calendar-entries">{entriesOn(day).map((entry) => <div className={`hr-calendar-entry ${entry.personKind}`} key={entry.id}><span>{entry.personName}</span><small>{entry.shiftName} · {entry.scopeName}</small>{canWrite && !version?.locked ? <button type="button" aria-label={`移除 ${entry.personName}`} onClick={() => setDraftEntries((current) => current.filter((candidate) => candidate.id !== entry.id))}>×</button> : null}</div>)}</div>
        </div>)}
      </div>
    </Panel>
