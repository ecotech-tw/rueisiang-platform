...
import { useHrQuery, useHrWrite, type Candidate, type Employee, type NamedOption, type Profile } from "./api.js";
  ...
  return <div className="page">
    ...
    <section className="panel p-6">
      <div className="flex flex-wrap gap-3">
        ...
        <Button variant="secondary" onClick={() => { void employees.refetch(); void candidates.refetch(); void scopes.refetch(); if (selected) void detail.refetch(); }}>重新整理</Button>
      </div>
      {employees.isPending ? <p>載入中…</p> : null}
      {[employees.error, candidates.error, scopes.error, detail.error].map((error, index) => error ? <Alert key={index} tone="danger">{error.message}</Alert> : null)}
    ...
    {profile ? <section className="panel p-6">
      ...
      {canWrite ? <>
        <div className="flex flex-wrap gap-3">
          ...
          <Button disabled={!activeEmployment || !scopes.data?.scopes.length} onClick={() => setEditor({ title: "新增櫃點指派", path: "/assignments", method: "POST", initial: { employmentId: activeEmployment?.id, validFrom: activeEmployment?.hiredOn }, description: "指派期間需在任職期間內；指派不會授予平台管理權限。", fields: [{ key: "scopeId", label: "櫃點", options: scopes.data?.scopes ?? [] }, { key: "validFrom", label: "起日", type: "date" }, { key: "validTo", label: "迄日（不含，可留空）", type: "date", optional: true }] })}>新增櫃點</Button>
        </div>
        {profile.employments.map((job) => <div key={job.id} className="mt-4">
          <h3>任職：{job.hiredOn}{job.endedOn ? `～${job.endedOn}` : "～目前"}</h3>
          <div className="flex flex-wrap gap-3">
            {!job.endedOn ? <Button variant="secondary" onClick={() => setEditor({ title: "結束任職", path: `/employments/${job.id}/end`, method: "PATCH", initial: { revision: job.revision }, description: "請先結束所有超過離職日期的櫃點指派。結束後仍保留歷史紀錄。", fields: [{ key: "endedOn", label: "不再任職首日", type: "date" }] })}>結束任職</Button> : null}
    ...
    {editor ? <EditorDialog editor={editor} onClose={() => { setEditor(null); void employees.refetch(); void candidates.refetch(); if (selected) void detail.refetch(); }} /> : null}
  </div>;
}

export function HrSelf() {
  usePageTitle("我的人事資料");
  const query = useHrQuery<{ profile: Profile | null }>("/me");
  return <div className="page"><PageHeader title="我的人事資料" description="這裡顯示與你登入帳號相連的任職與櫃點資料；資料有誤請聯絡管理者。" />
    {query.isPending ? <p>載入中…</p> : query.error ? <Alert tone="danger">{query.error.message}</Alert> : query.data?.profile ? <section className="panel p-6"><HrProfileDetails profile={query.data.profile} /></section> : <Alert>尚未指派為員工，請聯絡管理者。</Alert>}
  </div>;
}
