...
const ATTENDANCE_TABS = [
  { label: "出勤紀錄", to: "/hr/attendance-records", permission: "hr:office:read" as const },
  { label: "出勤設定", to: "/hr/attendance-settings", permission: "hr:office:read" as const },
];
  ...
  return <div className="hr-module">
    <header className="hr-module-header">
      ...
      <nav className="hr-module-tabs" aria-label={isAttendance ? "出勤管理子頁面" : "HRIS 子頁面"}>
        {tabs.filter((tab) => permissions.has(tab.permission)).map((tab) => <NavLink key={tab.to} to={tab.to} end className={({ isActive }) => `hr-module-tab${isActive ? " active" : ""}`}>{tab.label}</NavLink>)}
      </nav>
    </header>
    <Outlet />
  </div>;
