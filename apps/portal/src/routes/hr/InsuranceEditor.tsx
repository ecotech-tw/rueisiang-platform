...
export function InsuranceEditor({ employment, existing, defaultSalary, defaultDependentCount, onClose }: { employment: Employment; existing: boolean; defaultSalary?: number; defaultDependentCount?: number; onClose: () => void }) {
  ...
  return <Dialog title={existing ? "編輯勞健保" : "新增加保資料"} titleMeta="勞保與健保一起建立版本" className="hr-insurance-dialog" onClose={onClose} closeDisabled={save.isPending} formProps={{ onSubmit: (event) => {
    ...
    {status === "enrolled" ? <>
      <TextField label="實際月薪（元）" type="number" min="0" step="1" value={salary} required onChange={(event) => setSalary(event.target.value)} hint="投保級距會依月薪自動帶入，也可以從下拉選單選擇其他級距。" />
      <div className="form-grid two">
        {SCHEMES.map((scheme) => {
          ...
          return <section className="hr-insurance-scheme-card" key={scheme}>
            ...
            <SelectField label={`${INSURANCE_LABEL[scheme]}投保級距`} value={selection} options={bracketOptions} onChange={(event) => updateBracketSelection(scheme, event.target.value)} hint={customAmount ? "人工來源必須留下覆核備註。" : activeTable?.sourceKind === "manual" ? "目前套用人工維護級距，保存時會標示人工來源。" : "可依實際月薪自動帶入，或直接選擇其他級距。"} />
            {customAmount ? <TextField label="人工投保金額（元）" type="number" min="0" step="1" value={drafts[scheme].manualAmount} required onChange={(event) => updateDraft(scheme, { manualAmount: event.target.value })} /> : <p className="hr-insurance-bracket-summary">{selectedBracket ? `投保金額／${amountLabel(selectedBracket.insuredAmount)}` : activeTable ? "請輸入月薪以自動帶入級距。" : `尚未啟用${INSURANCE_LABEL[scheme]}級距。`}</p>}
