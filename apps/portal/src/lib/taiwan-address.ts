/**
 * 縣市與鄉鎮市區。從 rueisiang-crm 原封不動搬過來——同一份資料在兩邊各維護一份
 * 只會漂移，之後 CRM 下線就只剩這裡。
 *
 * **地址仍然存成單一字串**（customers.address），不拆成三個欄位。理由是
 * CYBERBIZ 回來的地址本來就是一整串，拆欄位就得在每次同步時猜它的結構，猜錯
 * 就把資料弄髒。表單用 compose/split 在編輯的當下拆開與組合，存進去的還是那一串。
 */
...
export function composeTaiwanAddress(city: string, district: string, addressLine: string): string {
  return [city.trim(), district.trim(), addressLine.trim()].filter(Boolean).join(" ");
}

export function splitTaiwanAddress(address: string): { city: string; district: string; addressLine: string } {
  const normalized = address.trim().replace(/台/g, "臺").replace(/\s+/g, " ");
  const city = (Object.keys(taiwanDistricts) as TaiwanCity[]).find((value) => normalized.startsWith(value)) ?? "";
  const afterCity = city ? normalized.slice(city.length).trim() : normalized;
  const district = city
    ? taiwanDistricts[city].find((value) => afterCity.startsWith(value)) ?? ""
    : "";
  const addressLine = district ? afterCity.slice(district.length).trim() : afterCity;
