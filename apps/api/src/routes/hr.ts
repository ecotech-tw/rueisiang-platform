...
import {
  ...
  correctHrCompensationVersion, createHrCompensationVersion, voidHrCompensationVersion, createHrEmployment, createHrFormRequest, createHrInsuranceVersions, endHrAssignment, endHrAttendanceLocationAssignment, endHrEmployment, getHrAttendanceLocation, getHrAttendanceLocationSchedules, getHrClockCalendar, getHrClockMapCenters, getHrOverview,
...
export const hr = new Hono<AppEnv>()
  ...
  .post("/employments/:id/compensation/:versionId/correct", requirePermission("hr:employee:write"), async (c) => {
    if (!await isHrAdministrator(c.get("db"), c.get("user").id)) throw new HTTPException(403, { message: "只有全平台 HR 管理者可以管理薪資資料。" });
    const input = await body(c);
    const validFrom = date(input, "validFrom")!;
    const validTo = date(input, "validTo", true);
    period(validFrom, validTo);
    return c.json(await correctHrCompensationVersion(c.get("db"), c.req.param("id"), c.req.param("versionId"), {
      employmentId: c.req.param("id"), validFrom, validTo, payBasis: payBasis(input),
      baseAmountMinor: integerValue(input, "baseAmountMinor", "薪資金額（分）", 0, Number.MAX_SAFE_INTEGER), note: noteValue(input), items: compensationItems(input),
    }, c.get("user")), 201);
  })
