...
describe("HR 薪資與出勤設定 migration", () => {
  it("不重建既有 HR 表，並為既有任職回填主要辦公位置", () => {
    ...
    try {
      ...
      for (const file of readdirSync(directory).filter((file) => file.endsWith(".sql") && file < "0135").sort()) apply(sqlite, file);
      ...
      apply(sqlite, "0135_sticky_dracula.sql");
      apply(sqlite, "0136_backfill_hr_attendance_settings.sql");
