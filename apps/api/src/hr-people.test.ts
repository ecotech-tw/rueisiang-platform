...
describe("HR 員工基礎", () => {
  ...
  it("管理範圍只授權指定櫃點的員工資料，且不會順便授予功能權限", async () => {
    await assign("self");
    await assign("other", "E002");
    const selfEmployment = await firstEmployment("self");
    const otherEmployment = await firstEmployment("other");
    await created("/hr/assignments", { employmentId: selfEmployment, scopeId: "scope", validFrom: "2026-01-01" });
    await db.insert(scopes).values({ id: "other-scope", sourceType: "manual", scopeKind: "store", name: "其他櫃點", normalizedName: "其他櫃點" });

    expect((await request("/hr/employees", "GET", undefined, "writer")).status).toBe(200);
    expect((await (await request("/hr/employees", "GET", undefined, "writer")).json() as { employees: unknown[] }).employees).toEqual([]);
    expect((await request("/hr/management-scopes", "POST", { userId: "writer", scopeId: "scope" })).status).toBe(201);
    const assignments = await (await request("/hr/management-scopes", "GET", undefined, "admin")).json() as { assignments: { userId: string; scopeId: string }[] };
    expect(assignments.assignments).toEqual(expect.arrayContaining([expect.objectContaining({ userId: "writer", scopeId: "scope" })]));

    const visible = await (await request("/hr/employees", "GET", undefined, "writer")).json() as { employees: { userId: string }[] };
    expect(visible.employees.map((employee) => employee.userId)).toEqual(["self"]);
    const visibleScopes = await (await request("/hr/scopes", "GET", undefined, "writer")).json() as { scopes: { id: string }[] };
    expect(visibleScopes.scopes.map((scope) => scope.id)).toEqual(["scope"]);
    expect((await request("/hr/employees/self", "GET", undefined, "writer")).status).toBe(200);
    expect((await request("/hr/employees/other", "GET", undefined, "writer")).status).toBe(404);
    expect((await request("/hr/employees/self", "PATCH", { employeeNumber: "E001-X", revision: 1 }, "writer")).status).toBe(200);
    expect((await request("/hr/employees/other", "PATCH", { employeeNumber: "E002-X", revision: 1 }, "writer")).status).toBe(404);
    expect((await request("/hr/employments", "POST", { userId: "other", hiredOn: "2027-01-01", seniorityStartOn: "2027-01-01" }, "writer")).status).toBe(403);
    expect((await request("/hr/assignments", "POST", { employmentId: selfEmployment, scopeId: "other-scope", validFrom: "2026-01-01" }, "writer")).status).toBe(404);
    expect((await request(`/hr/employments/${otherEmployment}/end`, "PATCH", { endedOn: "2027-01-01", revision: 1 }, "writer")).status).toBe(404);
    expect((await request("/hr/candidates", "GET", undefined, "writer")).status).toBe(403);

    expect((await request("/hr/management-scopes", "DELETE", undefined, "admin")).status).toBe(404);
    expect((await request("/hr/management-scopes/writer/scope", "DELETE", undefined, "admin")).status).toBe(200);
    expect((await request("/hr/employees/self", "GET", undefined, "writer")).status).toBe(404);
  });

  it("補打卡審核遵守管理範圍，指定審核者仍可看自己的待審單", async () => {
    await assign("self");
    await assign("other", "E002");
    const selfEmployment = await firstEmployment("self");
    await created("/hr/assignments", { employmentId: selfEmployment, scopeId: "scope", validFrom: "2026-01-01" });
    await created("/hr/management-scopes", { userId: "writer", scopeId: "scope" });
    await request("/hr/employees/self/supervisor", "PATCH", { supervisorUserId: "other", revision: 1 });
    const firstRequest = await created("/hr/me/form-requests", { correctionDate: taipeiToday(), requestedTime: "09:00", requestedEventKind: "clock_in", reason: "範圍審核測試。", approverUserId: null }, "self");
    await request(`/hr/me/form-requests/${firstRequest}/submit`, "POST", {}, "self");

    const reviewList = await (await request("/hr/me/form-requests", "GET", undefined, "writer")).json() as { reviewRequests: { id: string }[] };
    expect(reviewList.reviewRequests.map((item) => item.id)).toContain(firstRequest);
    expect((await request(`/hr/me/form-requests/${firstRequest}/review`, "POST", { decision: "approved", comment: "範圍內核准。" }, "writer")).status).toBe(200);

    await request("/hr/employees/other/supervisor", "PATCH", { supervisorUserId: "self", revision: 1 });
    const secondRequest = await created("/hr/me/form-requests", { correctionDate: taipeiToday(), requestedTime: "10:00", requestedEventKind: "clock_in", reason: "非範圍審核測試。", approverUserId: null }, "other");
    await request(`/hr/me/form-requests/${secondRequest}/submit`, "POST", {}, "other");
    const after = await (await request("/hr/me/form-requests", "GET", undefined, "writer")).json() as { reviewRequests: { id: string }[] };
    expect(after.reviewRequests.map((item) => item.id)).not.toContain(secondRequest);
    expect((await request(`/hr/me/form-requests/${secondRequest}/review`, "POST", { decision: "approved", comment: "不應可審。" }, "writer")).status).toBe(404);
  });

  it("HR 稽核只經獨立權限路徑回傳摘要，不會混入共用操作紀錄", async () => {
    await assign("self");
    const audit = await request("/hr/audit", "GET", undefined, "admin");
    expect(audit.status).toBe(200);
    const body = await audit.json() as { events: { source?: string; eventType: string; summary: string; payloadJson?: string }[] };
    expect(body.events.some((event) => event.eventType === "employee_assigned")).toBe(true);
    expect(body.events.every((event) => !Object.prototype.hasOwnProperty.call(event, "payloadJson"))).toBe(true);
    expect((await request("/hr/audit", "GET", undefined, "writer")).status).toBe(403);
    expect((await request("/crm/events", "GET", undefined, "admin")).status).toBe(200);
  });
